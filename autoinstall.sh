...
# Remove Hyperlane and Logs
remove_hyperlane() {
    log "${YELLOW}Removing Hyperlane components, Docker containers, images, and logs...${NC}"

    if docker ps -a --format '{{.Names}}' | grep -q "^hyperlane$"; then
        docker rm -f hyperlane || error_exit "Failed to remove the 'hyperlane' container."
        log "${GREEN}'hyperlane' container removed.${NC}"
    else
        log "${YELLOW}No 'hyperlane' container found. Skipping container removal.${NC}"
    fi

    if docker images | grep -q 'gcr.io/abacus-labs-dev/hyperlane-agent'; then
        docker rmi gcr.io/abacus-labs-dev/hyperlane-agent:agents-v1.0.0 || error_exit "Failed to remove Hyperlane image."
        log "${GREEN}Hyperlane image removed.${NC}"
    else
        log "${YELLOW}No Hyperlane image found. Skipping image removal.${NC}"
    fi

    if [ -d "$DB_DIR" ]; then
        rm -rf "$DB_DIR" || error_exit "Failed to remove database directory: $DB_DIR"
        log "${GREEN}Database directory removed: $DB_DIR${NC}"
    else
        log "${YELLOW}No database directory found. Skipping removal.${NC}"
    fi

    if [ -f "$LOG_FILE" ]; then
        rm -f "$LOG_FILE" || error_exit "Failed to remove log file: $LOG_FILE"
        log "${GREEN}Log file removed: $LOG_FILE${NC}"
    else
        log "${YELLOW}No log file found. Skipping log removal.${NC}"
    fi

    log "${GREEN}Hyperlane, Docker containers, images, and related logs have been removed successfully.${NC}"
}
...
main_menu() {
    while true; do
        ...
        echo "7) Remove Hyperlane and Logs"
        echo "0) Exit"
        echo "================================================================="
        echo -e "${NC}"
